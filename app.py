...
collection = db["bibliografia_neta"]

@app.route('/')
def home():
    return render_template('index.html')  # Servir el archivo HTML

@app.route('/cerca_avamcada_bona')
def cerca_avamcada_bona():
    return render_template('cerca_avamcada_bona.html')  # Servir el archivo HTML

@app.route('/bibliografia')
def bibliografia():
    return render_template('bibliografia.html')  # Servir el archivo HTML

@app.route('/sobre_larxiu_digital')
def sobre_larxiu_digital():
    return render_template('sobre_larxiu_digital.html')  # Servir el archivo HTML
...
def search():
    try:
        ...
        # Retornar si no se proporciona ningún criterio de búsqueda
        if not query and not optional_query and not source and not type_ and not publisher and not start_date and not end_date and not city:
            return jsonify({
                "total_documents": 0,
                "total_pages": 0,
                "current_page": page,
        ...
        if query or optional_query:
            ...
            # Asegura que el pipeline no devuelva documentos si no hay criterios de búsqueda
            query_pipeline.append({"$match": {"_id": {"$exists": False}}})
